...
with lib;
{
  options.custom = {
    ...
    hostname = mkOption {
      type = types.str;
      default = "";
      description = "The machine hostname (informational; hosts set networking.hostName from the specialArg)";
    };
    nativeNix = mkOption {
      type = types.bool;
      default = false;
    ...
    editor = mkOption {
      type = types.str;
      default = "vim";
      description = "Default $EDITOR (shared base).";
    };
    shellAliases = mkOption {
      type = types.attrsOf types.str;
      default = {
        sudo = "sudo ";
      }; # trailing space → alias-expand the next word
      description = "System shell aliases set by the shared base.";
    };
    enableDirenv = mkOption {
      type = types.bool;
      default = true;
      description = "Enable direnv + nix-direnv and the zsh hook (shared base).";
    };
    enableGnupgAgent = mkOption {
      type = types.bool;
      default = true;
      description = "Enable the GnuPG agent (shared base).";
    };
    fonts = mkOption {
